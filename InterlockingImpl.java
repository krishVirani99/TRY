 ...
 *
 * NOTE:
 *  - Do NOT ship Direction.java / TrainType.java / Interlocking.java. The grader provides them.
 *  - SectionGraph supplies adjacency, entries/exits, and line separation.
 */
public class InterlockingImpl implements Interlocking {
    ...
    // ============== OPTIONAL API (4-arg) ==============
    // Some graders/interfaces also declare this version. Include it to be safe.
    public boolean addTrain(String trainName, TrainType type, Direction dir, int entrySection) {
        ...
        try {
            ...
            if (dir == null)                              return false;
            if (!graph.sections.contains(entrySection))   return false;
            if (!isEntry(dir, entrySection))              return false;
            if (occ.containsKey(entrySection))            return false;

            // No explicit exit given here: set exit=-1 and allow leaving at any exit node.
            String line = lineOf(entrySection);
        ...
        try {
            for (String name : trainNames) {
                ...
                for (Integer nxt : options) {
                    ...
                    if (occ.containsKey(nxt))          continue; // free?
                    if (!graph.sameLine(t.at, nxt))     continue; // no mixing lines
                ...
                if (isExit(t.dir, t.at) && (t.exit < 0 || t.at == t.exit)) {
                    moved++;
                    t.at = -1; // do not occupy the exit section
                } else {
                    occ.put(t.at, name);
                    moved++;
                }
